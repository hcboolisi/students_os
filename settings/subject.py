    ...
    def add(self):
        ...
        if subID != "" and subName != "":
            if self.getName(subName) > 0:
               ...
               QMessageBox.information(None, "提示", "您要添加的科目已存在，请重新输入！",
                                     QMessageBox.Ok)
            else:
                result = service.exec("insert into tb_subject(subID,subName) values (%s,%s)",
                                      (subID, subName))
                if result > 0:
                    self.query()
        ...
        try:
            if self.select != "":
                ...
                if subName != "":
                    if self.getName(subName) > 0:
                        self.lineEdit_2.setText("")
                        QMessageBox.information(None, '提示', '您要修改的科目已存在，请重新输入！',
                                                QMessageBox.Ok)
                    else:
